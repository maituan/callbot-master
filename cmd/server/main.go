	...
	if pgStore != nil {
		if n, err := pgStore.CountBots(rootCtx); err != nil {
			...
			botID, err := pgStore.SeedDefaultBot(rootCtx, store.SeedBotInput{
				...
				ASRSilenceTimeoutSec:  float64(cfg.ASR.SilenceTimeout) / float64(time.Second),
				ASRSpeechTimeoutSec:   float64(cfg.ASR.SpeechTimeout) / float64(time.Second),
				ASRSpeechMaxSec:       float64(cfg.ASR.SpeechMax) / float64(time.Second),
